...
Public API:
    ...
    RequestInput.from_sample   — builds a RequestInput from a sample and config.
    RequestInput.build_all     — pre-builds all RequestInputs from a sample list.
    RequestInput.send          — sends one chat-completions SSE request and returns a RequestOutput.
    RequestInput.dispatch_all  — schedules and fires all requests with arrival timing, returns outputs.
"""
...
class RequestInput:
    ...
    @classmethod
    def from_sample(
        cls, config: BenchmarkConfig, sample: SampleRequest, api_url: str
    ) -> RequestInput:
    ...
    def build_all(
        ...
        return [cls.from_sample(config, s, api_url) for s in samples]
    ...
    async def send(
        ...
        try:
            return await self._send(session, tracker)
        except Exception:
    ...
    async def _send(
        ...
        if isinstance(self.prompt, str):
            ...
            content = [{"type": "text", "text": self.prompt[0]}]
            messages = [{"role": "user", "content": content}]
            for i, turn in enumerate(self.prompt[1:]):
        ...
        payload: dict[str, Any] = {
            "model": self.model_id,
            "messages": messages,
            "max_completion_tokens": self.output_len,
            "stream": True,
        ...
        output = RequestOutput(prompt=self.prompt, prompt_len=self.prompt_len)
        output_text = ""
        reasoning_text = ""
        ttft = 0.0
    ...
    async def dispatch_all(
        ...
        async for req in cls._iter_spaced(requests, request_rate, burstiness):
            async def _send(r: RequestInput = req) -> RequestOutput:
                async with semaphore:
                    return await r.send(session, tracker)
