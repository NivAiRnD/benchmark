...
class Benchmark:
    ...
    class BenchmarkManagedDependencies:
    ...
    class BenchmarkContext:
        ...
        BenchmarkManagedDependencies infrastructure.  The window is intentionally
        tight: it starts the moment requests are dispatched and ends when all tasks
        complete, so energy and timing measurements exclude server startup and idle
        overhead.
        ...
        def __init__(
            ...
            deps: Benchmark.BenchmarkManagedDependencies,
        ...
        async def __aenter__(self) -> Benchmark.BenchmarkContext:
    ...
    def __init__(
        ...
        self._deps = Benchmark.BenchmarkManagedDependencies(
            config, log_path, max_num_seqs, max_num_batched_tokens, monitor_cpu_power
        )
    ...
    async def run(self, requests: list[SampleRequest]) -> BenchmarkResult:
        ...
        async with Benchmark.BenchmarkContext(
            requests, self._deps, self._tokenizer, self._config.traffic.max_concurrency
        ) as ctx:
